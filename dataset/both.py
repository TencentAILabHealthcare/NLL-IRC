...
class DataLoader_both: #FastTensorDataLoader_onehotEncode_onebatch:
        ...
        def __next__(self):
                ...
                batch['seq2']= torch.LongTensor([strtolist(i[1],self.dataaug) for i in current_data]) 

                #print(batch['seq'])

                batch['tcr']=torch.from_numpy(self.seq_encode[self.r[self.index:self.index+self.batch_size]]) 
                #if self.in_memory:
