...
class Tokenizer(nn.Module):
    ...
    def __init__(
        self,
        d_numerical: ty.Optional[ty.List[int]],
        ...
        if categories is None:
            d_bias = len(d_numerical)
            self.category_offsets = None
            self.category_embeddings = None
        else:
            d_bias = len(d_numerical) + len(categories)
            category_offsets = torch.tensor([0] + categories[:-1]).cumsum(0)
            # print(category_offsets)
        ...
        def weights_init(m):
            if isinstance(m, nn.Linear):
                nn_init.kaiming_uniform_(m.weight.data, a=math.sqrt(5))

        if d_numerical is None:
            pass
        else:
            self.numerical_feats_to_token = nn.ModuleList([nn.Linear(i, d_token) for i in d_numerical]) 
            self.numerical_feats_to_token.apply(weights_init)
        self.CLS_token = nn.Parameter(Tensor(1, d_token))
    ...
    def n_tokens(self) -> int:
        return 1 + ( 0 if self.category_offsets is None else len(self.category_offsets)) + (0 if self.d_numerical is None else len(self.d_numerical))

    def forward(self, x_num: ty.Optional[ty.List[Tensor]], x_cat: ty.Optional[Tensor]) -> Tensor:
        x_some = x_num[0] if x_cat is None else x_cat
        assert x_some is not None
        assert len(x_num)==len(self.numerical_feats_to_token)
        # x_num = torch.cat(
        ...
        x=[]
        for i, j in zip(x_num,self.numerical_feats_to_token):
            x.append(j(i))  
        
        cls_embed=torch.ones(len(x_some), 1, device=x_some.device)*self.CLS_token

        x=torch.stack([cls_embed]+x,dim=1)
...
class Transformer_effecient(nn.Module):
    """Transformer.
    ...
    def __init__(
        ...
        *,
        ...
        def make_kv_compression():
            ...
            # print(n_tokens, int(n_tokens * kv_compression))
            # exit()
